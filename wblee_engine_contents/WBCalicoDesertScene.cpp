...
namespace wb
{
	WBCalicoDesertScene::WBCalicoDesertScene()
		:mPlayer(nullptr),
		mCamera(nullptr)
	{
	}
	...
	void WBCalicoDesertScene::Initialize()
	{
		// Camera
		...
		WBGameObject* map = object::Instantiate<WBGameObject>
			(enums::eLayerType::Map, Vector2(-400.f, -380.5f));
		WBSpriteRenderer* mapSr = map->AddComponent<WBSpriteRenderer>();

		graphics::WBTexture* mapTexture = WBResources::Find<graphics::WBTexture>(L"Calico_Desert");
	...
	void WBCalicoDesertScene::Update()
	{
		WBScene::Update();
	}

	void WBCalicoDesertScene::LateUpdate()
	{
		WBScene::LateUpdate();
	...
	void WBCalicoDesertScene::Render(HDC hdc)
	{
	...
	void WBCalicoDesertScene::OnEnter()
	{
		renderer::mainCamera = mCamera->GetComponent<WBCamera>();
	}

	void WBCalicoDesertScene::OnExit()
	{
	}
}