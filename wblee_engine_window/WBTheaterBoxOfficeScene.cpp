...
namespace wb
{
	WBTheaterBoxOfficeScene::WBTheaterBoxOfficeScene()
		:mPlayer(nullptr),
		mCamera(nullptr)
	{
	}
	...
	void WBTheaterBoxOfficeScene::Initialize()
	{
		// Camera
		...
		// Stardrop Saloon map
		WBGameObject* map = object::Instantiate<WBGameObject>(enums::eLayerType::Map);
		graphics::WBTexture* mapTexture = WBResources::Find<graphics::WBTexture>(L"Theater_Box_Office");
	...
	void WBTheaterBoxOfficeScene::Update()
	{
		WBScene::Update();
	}

	void WBTheaterBoxOfficeScene::LateUpdate()
	{
		WBScene::LateUpdate();

		if (WBInput::GetKeyDown(eKeyCode::W))
		{
			WBSceneManager::LoadScene(L"TheaterScreenScene");
		}
	...
	void WBTheaterBoxOfficeScene::Render(HDC hdc)
	{
	...
	void WBTheaterBoxOfficeScene::OnEnter()
	{
		renderer::mainCamera = mCamera->GetComponent<WBCamera>();
	}

	void WBTheaterBoxOfficeScene::OnExit()
	{
	}
}